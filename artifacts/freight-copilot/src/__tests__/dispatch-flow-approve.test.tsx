...
describe('Dispatch flow - approve', () => {
  it('approves and dispatches load FC-2048, decreases pending count, and updates load decision', async () => {
    ...
    const approveButton = screen.getByTestId('button-approve-load');
    expect(approveButton).not.toBeDisabled();
    await user.click(approveButton);

    // Wait for toast confirmation
    await waitFor(() => {
      expect(screen.getByTestId('status-toast')).toHaveTextContent('FC-2048 approved and moved to dispatch');
    }, { timeout: 10000 });

    // Check pending count decreased in sidebar nav (queue nav shows pending count)
    await waitFor(() => {
